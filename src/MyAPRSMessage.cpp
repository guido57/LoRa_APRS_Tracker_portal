...
String APRSMessage::getLatitude() const
{
	return _lat;
}

String APRSMessage::getLongitude() const
{
	return _lng;
}
...
bool APRSMessage::decode(const String & message)
{
	...
	if(pos_dest != -1 && pos_dest < pos_path)
	{
		...
		_lat = message.substring(pos_path+2,pos_path+10);
		_lng = message.substring(pos_path+11,pos_path+20);
	}
	else
	{
		_path = "";
